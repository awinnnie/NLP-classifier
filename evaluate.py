...
le = LabelEncoder()
y_val_enc = le.fit_transform(y_val) #for xgboost
...
for file in os.listdir("Models"):
    if file.endswith(".pkl"):
        ...
        pipeline = pickle.load(open(os.path.join("Models", file), "rb"))
        
        if name == "xgboost":
            y_pred_enc = pipeline.predict(X_val)
            y_pred = le.inverse_transform(y_pred_enc)
            y_score = pipeline.predict_proba(X_val) #ROC
        else:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join("Results\\confusion_matrices", f"{name}_cm.png"))
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(os.path.join("Results\\roc_curves", f"{name}_roc.png"))
        plt.close()


results_df = pd.DataFrame(results).sort_values("Accuracy", ascending=False)
results_df.to_csv(os.path.join("Results", "metrics.csv"), index=False)
print("\nModel Performance\n")
